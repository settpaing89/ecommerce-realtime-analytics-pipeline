...
def transform_customers(df):
    ...
    if "date_of_birth" in df.columns:
        ...
        df["age"] = (
            ((datetime.now() - df["date_of_birth"]).dt.days // 365)
            .fillna(0)
            .astype(int)
        )
    ...
    # Data quality flags
    df["dq_email_valid"] = (
        df["email"].str.contains("@", na=False) if "email" in df.columns else False
    )
...
def transform_products(df):
    ...
    if "base_price" in df.columns and "current_price" in df.columns:
        df["discount_pct"] = (
            (df["base_price"] - df["current_price"]) / df["base_price"] * 100
        ).round(2)

    # Calculate profit margin
    if "current_price" in df.columns and "cost" in df.columns:
        df["profit_margin"] = (
            (df["current_price"] - df["cost"]) / df["current_price"] * 100
        ).round(2)

    # Data quality
    df["dq_has_inventory"] = (
        df["inventory_quantity"] > 0 if "inventory_quantity" in df.columns else False
    )
...
def transform_orders(df):
    ...
    if (
        "unit_price" not in df.columns
        and "subtotal" in df.columns
    ...
    df["dq_valid_status"] = df["status"].isin(
...
def process_data_type(data_type, bronze_key):
    ...
    try:
        ...
        s3.put_object(
            Bucket=SILVER_BUCKET, Key=silver_key, Body=parquet_buffer.getvalue()
        )
