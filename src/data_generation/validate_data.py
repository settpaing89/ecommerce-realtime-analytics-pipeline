...
def validate_dataset(df, name, required_columns, unique_columns, allow_nulls=None):
    ...
    unexpected_nulls = {col: count for col, count in null_counts.items() 
...
def main():
    ...
    results.append(validate_dataset(
        ...
        unique_columns=['customer_id'],  # Email can have duplicates (rare but possible)
        allow_nulls=[]
    ))
    ...
    results.append(validate_dataset(
        orders,
        'Orders',
        required_columns=['order_id', 'customer_id', 'product_id', 'order_date', 'total_amount'],
    ...
    if all(results):
        ...
        print(f"  • Total: {len(customers) + len(products) + len(orders) + len(events):,} records")
